...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if resetTimestamp == "" {
		...
		case PlatformOpenAI:
			...
			if s.isOpenAIOAuthYield429Enabled() && classifyOpenAIOAuth429Usage(responseBody, time.Now()).HardUsageLimit {
...
// unavailable on the account when the upstream returns a deterministic 404
// model-not-found response. Ambiguous Codex plan-gated 400 responses are
// handled by the gateway's adaptive transient state instead of being persisted.
func (s *RateLimitService) HandleUpstreamModelNotFound(ctx context.Context, account *Account, requestedModel string, statusCode int, responseBody []byte) bool {
	...
	if err := s.accountRepo.SetModelRateLimit(ctx, account.ID, modelKey, resetAt, upstreamModelNotFoundReason); err != nil {
		slog.Warn("upstream_model_not_found_set_model_rate_limit_failed", "account_id", account.ID, "model", modelKey, "reason", upstreamModelNotFoundReason, "error", err)
		return true
	}
	slog.Info("upstream_model_not_found_model_rate_limited", "account_id", account.ID, "model", modelKey, "reason", upstreamModelNotFoundReason, "reset_at", resetAt)
	return true
}
