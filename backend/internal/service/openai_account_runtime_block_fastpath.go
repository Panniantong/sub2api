...
func (s *OpenAIGatewayService) handleOpenAIAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, canonicalModel ...string) bool {
	...
	if ordinaryOpenAIOAuth429 {
		oauth429Classification = classifyOpenAIOAuth429Usage(responseBody, time.Now())
...
func classifyOpenAIOAuth429Usage(responseBody []byte, now time.Time) openAIOAuth429Classification {
...
func (s *OpenAIGatewayService) markOpenAIOAuth429RateLimited(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if resetAt != nil && resetAt.After(now) {
		...
		if s.isOpenAIOAuthYield429Enabled() {
			if bodyResetAt := classifyOpenAIOAuth429Usage(responseBody, now).ResetAt; bodyResetAt != nil {
				cooldownUntil = *bodyResetAt
			}
