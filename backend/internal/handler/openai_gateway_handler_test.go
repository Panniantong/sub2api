...
func TestOpenAIResponsesWebSocket_PlainWorkModeAliasUsesBaseUpstreamAndPreservesRequestedModel(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload: `{"type":"response.create","model":"gpt-5.6","stream":false}`,
	})

	require.Len(t, got.upstreamPayloads, 1)
	require.Equal(t, "gpt-5.6-sol", gjson.GetBytes(got.upstreamPayloads[0], "model").String())
	require.Len(t, got.clientEvents, 1)
	require.Equal(t, "gpt-5.6", gjson.GetBytes(got.clientEvents[0], "response.model").String())
	require.Len(t, got.logs, 1)
	require.Equal(t, "gpt-5.6", got.logs[0].RequestedModel)
	require.NotNil(t, got.logs[0].UpstreamModel)
	require.Equal(t, "gpt-5.6-sol", *got.logs[0].UpstreamModel)
}
