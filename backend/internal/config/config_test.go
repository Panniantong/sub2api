...
func TestLoadOpenAIOAuthFillFirstConfig(t *testing.T) {
	t.Run("disabled by default", func(t *testing.T) {
		resetViperWithJWTSecret(t)
		cfg, err := Load()
		require.NoError(t, err)
		require.False(t, cfg.Gateway.OpenAIScheduler.OAuthFillFirstEnabled)
	})

	t.Run("enabled by instance environment", func(t *testing.T) {
		resetViperWithJWTSecret(t)
		t.Setenv("GATEWAY_OPENAI_SCHEDULER_OAUTH_FILL_FIRST_ENABLED", "true")
		cfg, err := Load()
		require.NoError(t, err)
		require.True(t, cfg.Gateway.OpenAIScheduler.OAuthFillFirstEnabled)
	})
}
